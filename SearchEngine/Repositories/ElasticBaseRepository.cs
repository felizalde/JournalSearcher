...
public abstract class ElasticBaseRepository<T> : IElasticBaseRepository<T> where T : BaseIndex
{
    ...
    public async Task<IEnumerable<T>> GetAllAsync()
    {
        ...
        return response.Hits
                    .Select(hit => hit.Source).ToList();
    }

    public async Task<IEnumerable<JournalResult<T>>> SearchAsync(Func<QueryContainerDescriptor<T>, QueryContainer> request)
    {
    ...
    public async Task<IEnumerable<JournalResult<T>>> SearchAsync(Func<SearchDescriptor<T>, ISearchRequest> selector)
    {
        var list = new List<T>();
    ...
    public async Task<bool> DeleteAllAsync(IEnumerable<T> documents)
    {
        var response = await _elasticClient.DeleteManyAsync(documents, this.IndexName);
